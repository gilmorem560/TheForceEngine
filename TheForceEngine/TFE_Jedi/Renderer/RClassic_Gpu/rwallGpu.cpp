...
namespace RClassic_Gpu
{
	...
	f32 frustumIntersect(f32 x0, f32 z0, f32 x1, f32 z1, f32 dx, f32 dz)
	{
		f32 xz;
		xz = (x0 * z1) - (z0 * x1);
		f32 dyx = dz * s_rcgpuState.nearPlaneHalfLen - dx;
		if (dyx != 0.0f)
		{
			xz /= dyx;
		}
	...
	bool wall_clipToFrustum(f32& x0, f32& z0, f32& x1, f32& z1, f32& dx, f32& dz, f32& curU, f32& texelLen, f32& texelLenRem, s32& clipX0_Near, s32& clipX1_Near, f32 left0, f32 right0, f32 left1, f32 right1)
	{
		...
		if (x0 < left0)
		{
			...
			{
				s = (-xz * s_rcgpuState.nearPlaneHalfLen - x0) / dx;
			}

			// Update the x0,y0 coordinate of the segment.
			x0 = -xz * s_rcgpuState.nearPlaneHalfLen;
			z0 = xz;
		...
		if (x1 > right1)
		{
			...
			{
				s = (xz*s_rcgpuState.nearPlaneHalfLen - x1) / dx;
			}

			// Update the x1,y1 coordinate of the segment.
			x1 = xz * s_rcgpuState.nearPlaneHalfLen;
			z1 = xz;
			if (s != 0)
			{
		...
		//////////////////////////////////////////////////
		if ((z0 < 0 || z1 < 0) && segmentCrossesLine(0.0f, 0.0f, 0.0f, -s_rcgpuState.halfHeight, x0, x0, x1, z1) != 0)
		{
			return false;
		}
		if (z0 < 1.0f && z1 < 1.0f)
	...
	void wall_process(WallCached* wallCached)
	{
		...
		f32 left0 = -z0 * s_rcgpuState.nearPlaneHalfLen;
		f32 left1 = -z1 * s_rcgpuState.nearPlaneHalfLen;
		f32 right0 = z0 * s_rcgpuState.nearPlaneHalfLen;
		f32 right1 = z1 * s_rcgpuState.nearPlaneHalfLen;
		...
		// Project.
		//////////////////////////////////////////////////
		f32 x0proj = (x0*s_rcgpuState.focalLength)/z0 + s_rcgpuState.projOffsetX;
		f32 x1proj = (x1*s_rcgpuState.focalLength)/z1 + s_rcgpuState.projOffsetX;
		...
		{
			x0 = -s_rcgpuState.nearPlaneHalfLen;
			dx = x1 + s_rcgpuState.nearPlaneHalfLen;
			x0pixel = s_minScreenX_Pixels;
		}
		if (clipX1_Near != 0 && x1pixel < s_maxScreenX_Pixels)
		{
			dx = s_rcgpuState.nearPlaneHalfLen - x0;
		...
		RWallSegmentFloat* wallSeg = &s_rcgpuState.wallSegListSrc[s_nextWall];
		s_nextWall++;
	...
	s32 wall_mergeSort(RWallSegmentFloat* segOutList, s32 availSpace, s32 start, s32 count)
	{
		...
		RWallSegmentFloat* srcSeg = &s_rcgpuState.wallSegListSrc[start];
		...
		while (1)
		{
			...
			JBool insideWindow = ((srcSeg->z0 >= s_rcgpuState.windowMinZ || srcSeg->z1 >= s_rcgpuState.windowMinZ) && srcSeg->wallX0 <= s_windowMaxX_Pixels && srcSeg->wallX1 >= s_windowMinX_Pixels) ? JTRUE : JFALSE;
	...
	void wall_drawSolid(RWallSegmentFloat* wallSegment)
	{
		...
		f32 ceilEyeRel  = ceilingHeight - s_rcgpuState.eyeHeight;
		f32 floorEyeRel = floorHeight   - s_rcgpuState.eyeHeight;

		f32 z0 = wallSegment->z0;
		f32 z1 = wallSegment->z1;

		f32 y0C = (ceilEyeRel  * s_rcgpuState.focalLenAspect) / z0 + s_rcgpuState.projOffsetY;
		f32 y1C = (ceilEyeRel  * s_rcgpuState.focalLenAspect) / z1 + s_rcgpuState.projOffsetY;
		f32 y0F = (floorEyeRel * s_rcgpuState.focalLenAspect) / z0 + s_rcgpuState.projOffsetY;
		f32 y1F = (floorEyeRel * s_rcgpuState.focalLenAspect) / z1 + s_rcgpuState.projOffsetY;
		...
		if (y0C_pixel > s_windowMaxY_Pixels && y1C_pixel > s_windowMaxY_Pixels)
		{
			...
			{
				s_rcgpuState.depth1d[x] = solveForZ(wallSegment, x, numerator);
	...
	void wall_drawMask(RWallSegmentFloat* wallSegment)
	{
		...
		{
			cProj0 = cProj1 = s_rcgpuState.windowMinY;
		}
		else
		{
			f32 ceilRel = cachedSector->ceilingHeight - s_rcgpuState.eyeHeight;
			cProj0 = ((ceilRel*s_rcgpuState.focalLenAspect)/z0) + s_rcgpuState.projOffsetY;
			cProj1 = ((ceilRel*s_rcgpuState.focalLenAspect)/z1) + s_rcgpuState.projOffsetY;
		}
		...
		{
			fProj0 = fProj1 = s_rcgpuState.windowMaxY;
		}
		else
		{
			f32 floorRel = cachedSector->floorHeight - s_rcgpuState.eyeHeight;
			fProj0 = ((floorRel*s_rcgpuState.focalLenAspect)/z0) + s_rcgpuState.projOffsetY;
			fProj1 = ((floorRel*s_rcgpuState.focalLenAspect)/z1) + s_rcgpuState.projOffsetY;
		}
	...
	void wall_drawBottom(RWallSegmentFloat* wallSegment)
	{
		...
		{
			cProj0 = s_rcgpuState.windowMinY;
			cProj1 = cProj0;
		}
		else
		{
		...
		if (cy0 > s_windowMaxY_Pixels && cy1 >= s_windowMaxY_Pixels)
		{
			...
			for (s32 i = 0; i < length; i++, x++)
			{
				s_rcgpuState.depth1d[x] = solveForZ(wallSegment, x, num);
				s_columnTop[x] = s_windowMaxY_Pixels;
			}
			srcWall->seen = JTRUE;
			return;
		...
		f32 floorRel = cachedSector->floorHeight - s_rcgpuState.eyeHeight;
		f32 fProj0 = (floorRel*s_rcgpuState.focalLenAspect)/z0 + s_rcgpuState.projOffsetY;
		f32 fProj1 = (floorRel*s_rcgpuState.focalLenAspect)/z1 + s_rcgpuState.projOffsetY;
		...
		f32 floorRelNext = fixed16ToFloat(nextSector->floorHeight) - s_rcgpuState.eyeHeight;
		f32 fNextProj0 = (floorRelNext*s_rcgpuState.focalLenAspect)/z0 + s_rcgpuState.projOffsetY;
		f32 fNextProj1 = (floorRelNext*s_rcgpuState.focalLenAspect)/z1 + s_rcgpuState.projOffsetY;
	...
	void wall_drawTop(RWallSegmentFloat* wallSegment)
	{
		...
		f32 ceilRel = cachedSector->ceilingHeight - s_rcgpuState.eyeHeight;
		f32 yC0 =((ceilRel*s_rcgpuState.focalLenAspect)/z0) + s_rcgpuState.projOffsetY;
		f32 yC1 =((ceilRel*s_rcgpuState.focalLenAspect)/z1) + s_rcgpuState.projOffsetY;
		...
		{
			f32 floorRel = cachedSector->floorHeight - s_rcgpuState.eyeHeight;
			yF0 = (floorRel*s_rcgpuState.focalLenAspect)/z0 + s_rcgpuState.projOffsetY;
			yF1 = (floorRel*s_rcgpuState.focalLenAspect)/z1 + s_rcgpuState.projOffsetY;
		}
		...
		f32 next_ceilRel = fixed16ToFloat(next->ceilingHeight) - s_rcgpuState.eyeHeight;
		f32 next_yC0 = (next_ceilRel*s_rcgpuState.focalLenAspect)/z0 + s_rcgpuState.projOffsetY;
		f32 next_yC1 = (next_ceilRel*s_rcgpuState.focalLenAspect)/z1 + s_rcgpuState.projOffsetY;
	...
	void wall_drawTopAndBottom(RWallSegmentFloat* wallSegment)
	{
		...
		f32 ceilRel = cachedSector->ceilingHeight - s_rcgpuState.eyeHeight;
		f32 cProj0 = (ceilRel*s_rcgpuState.focalLenAspect)/z0 + s_rcgpuState.projOffsetY;
		f32 cProj1 = (ceilRel*s_rcgpuState.focalLenAspect)/z1 + s_rcgpuState.projOffsetY;
		...
		RSector* nextSector = srcWall->nextSector;
		f32 next_ceilRel = fixed16ToFloat(nextSector->ceilingHeight) - s_rcgpuState.eyeHeight;
		f32 next_cProj0 = (next_ceilRel*s_rcgpuState.focalLenAspect)/z0 + s_rcgpuState.projOffsetY;
		f32 next_cProj1 = (next_ceilRel*s_rcgpuState.focalLenAspect)/z1 + s_rcgpuState.projOffsetY;
		...
		f32 next_floorRel = fixed16ToFloat(nextSector->floorHeight) - s_rcgpuState.eyeHeight;
		f32 next_fProj0 = (next_floorRel*s_rcgpuState.focalLenAspect)/z0 + s_rcgpuState.projOffsetY;
		f32 next_fProj1 = (next_floorRel*s_rcgpuState.focalLenAspect)/z1 + s_rcgpuState.projOffsetY;
	...
	void wall_drawSkyTop(RSector* sector)
	{
		...
		for (s32 x = s_windowMinX_Pixels; x <= s_windowMaxX_Pixels; x++)
		{
			...
			if (s_yPixelCount > 0)
			{
				if (s_height == SKY_BASE_HEIGHT)
				{
					s_vCoordFixed = floatToFixed20(f32(s_texHeightMask - y1) - s_rcgpuState.skyPitchOffset - fixed16ToFloat(sector->ceilOffset.z));
				}
				else
				{
					s_vCoordFixed = floatToFixed20(f32(s_texHeightMask) - (f32(y1)*heightScale) - s_rcgpuState.skyPitchOffset - fixed16ToFloat(sector->ceilOffset.z));
				}

				s32 texelU = (floorFloat(fixed16ToFloat(sector->ceilOffset.x) - s_rcgpuState.skyYawOffset + s_rcgpuState.skyTable[x]) ) & texWidthMask;
				s_texImage = &texture->image[texelU << texture->logSizeY];
				s_columnOut = &s_display[y0*s_width + x];
				drawColumn_Fullbright();
			}
	...
	void wall_drawSkyTopNoWall(RSector* sector)
	{
		...
		for (s32 x = s_windowMinX_Pixels; x <= s_windowMaxX_Pixels; x++)
		{
			...
			if (s_yPixelCount > 0)
			{
				...
				s32 widthMask = texture->width - 1;
				s32 texelU = floorFloat(fixed16ToFloat(sector->ceilOffset.x) - s_rcgpuState.skyYawOffset + s_rcgpuState.skyTable[x]) & widthMask;
				s_texImage = &texture->image[texelU << texture->logSizeY];
				s_columnOut = &s_display[y0*s_width + x];
	...
	void wall_drawSkyBottom(RSector* sector)
	{
		...
		for (s32 x = s_windowMinX_Pixels; x <= s_windowMaxX_Pixels; x++)
		{
			...
			if (s_yPixelCount > 0)
			{
				if (s_height == SKY_BASE_HEIGHT)
				{
					s_vCoordFixed = floatToFixed20(f32(s_texHeightMask - y1) - s_rcgpuState.skyPitchOffset - fixed16ToFloat(sector->floorOffset.z));
				}
				else
				{
					s_vCoordFixed = floatToFixed20(f32(s_texHeightMask) - (f32(y1)*heightScale) - s_rcgpuState.skyPitchOffset - fixed16ToFloat(sector->floorOffset.z));
				}

				s32 texelU = floorFloat(fixed16ToFloat(sector->floorOffset.x) - s_rcgpuState.skyYawOffset + s_rcgpuState.skyTable[x]) & texWidthMask;
				s_texImage = &texture->image[texelU << texture->logSizeY];
				s_columnOut = &s_display[y0*s_width + x];
				drawColumn_Fullbright();
			}
	...
	void wall_drawSkyBottomNoWall(RSector* sector)
	{
		...
		for (s32 x = s_windowMinX_Pixels; x <= s_windowMaxX_Pixels; x++)
		{
			...
			if (s_yPixelCount > 0)
			{
				...
				s32 widthMask = texture->width - 1;
				s32 texelU = floorFloat(fixed16ToFloat(sector->floorOffset.x) - s_rcgpuState.skyYawOffset + s_rcgpuState.skyTable[x]) & widthMask;
				s_texImage = &texture->image[texelU << texture->logSizeY];
				s_columnOut = &s_display[y0*s_width + x];
	...
	f32 solveForZ(RWallSegmentFloat* wallSegment, s32 x, f32 numerator, f32* outViewDx/*=nullptr*/)
	{
		...
		if (wallSegment->orient == WORIENT_DZ_DX)
		{
			...
			const f32 halfWidthOverX = ((fx != s_rcgpuState.halfWidth) ? s_rcgpuState.focalLength / (fx - s_rcgpuState.halfWidth) : s_rcgpuState.focalLength);
		...
		else  // WORIENT_DX_DZ
		{
			...
			// Note in the original code s_focalLength == s_halfWidth, so in that case the code is functionally equivalent.
			const f32 xOverHalfWidth = (f32(x) - s_rcgpuState.halfWidth) / s_rcgpuState.focalLength;
	...
	void wall_addAdjoinSegment(s32 length, s32 x0, f32 top_dydx, f32 y1, f32 bot_dydx, f32 y0, RWallSegmentFloat* wallSegment)
	{
		if (s_adjoinSegCount < MAX_ADJOIN_SEG)
		{
			...
			edgePair_setup(length, x0, top_dydx, y1End, y1, bot_dydx, y0, y0End, s_rcgpuState.adjoinEdge);

			s_rcgpuState.adjoinEdge++;
			s_adjoinSegCount++;

			*s_rcgpuState.adjoinSegment = wallSegment;
			s_rcgpuState.adjoinSegment++;
		}
	}
	...
	void sprite_drawFrame(u8* basePtr, WaxFrame* frame, SecObject* obj, vec3_float* cachedPosVS)
	{
		...
		const f32 projX0 = x0*s_rcgpuState.focalLength   *rcpZ + s_rcgpuState.projOffsetX;
		const f32 projY0 = y0*s_rcgpuState.focalLenAspect*rcpZ + s_rcgpuState.projOffsetY;
		...
		const f32 projX1 = x1*s_rcgpuState.focalLength   *rcpZ + s_rcgpuState.projOffsetX;
		const f32 projY1 = y1*s_rcgpuState.focalLenAspect*rcpZ + s_rcgpuState.projOffsetY;
