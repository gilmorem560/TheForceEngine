...
namespace RClassic_Gpu
{
	...
	void flat_addEdges(s32 length, s32 x0, f32 dyFloor_dx, f32 yFloor, f32 dyCeil_dx, f32 yCeil)
	{
		if (s_flatCount < MAX_SEG && length > 0)
		{
			...
			edgePair_setup(length, x0, dyFloor_dx, yFloor1, yFloor, dyCeil_dx, yCeil, yCeil1, s_rcgpuState.flatEdge);

			if (s_rcgpuState.flatEdge->yPixel_C1 - 1 > s_wallMaxCeilY)
			{
				s_wallMaxCeilY = s_rcgpuState.flatEdge->yPixel_C1 - 1;
			}
			if (s_rcgpuState.flatEdge->yPixel_F1 + 1 < s_wallMinFloorY)
			{
				s_wallMinFloorY = s_rcgpuState.flatEdge->yPixel_F1 + 1;
			}
	...
	{
		f32 textureOffsetU = s_rcgpuState.cameraPos.x - sectorCached->ceilOffset.x;
		f32 textureOffsetV = sectorCached->ceilOffset.z - s_rcgpuState.cameraPos.z;

		f32 relCeil          = sectorCached->ceilingHeight - s_rcgpuState.eyeHeight;
		f32 scaledRelCeil    =  relCeil * s_rcgpuState.focalLenAspect;
		f32 cosScaledRelCeil =  scaledRelCeil * s_rcgpuState.cosYaw;
		f32 negSinRelCeil    = -relCeil * s_rcgpuState.sinYaw;
		f32 sinScaledRelCeil =  scaledRelCeil * s_rcgpuState.sinYaw;
		f32 negCosRelCeil    = -relCeil * s_rcgpuState.cosYaw;
		...
		for (s32 y = s_windowMinY_Pixels; y <= s_wallMaxCeilY && y < s_windowMaxY_Pixels; y++)
		{
			...
			for (s32 i = 0; i < count;)
			{
				...
				if (s_scanlineWidth > 0)
				{
					...
					f32 rightClip = f32(right - s_screenXMid) * s_rcgpuState.aspectScaleX;
					...
					const f32 worldTexelScaleAspect = yRcp * worldToTexelScale * s_rcgpuState.aspectScaleY;
	...
	{
		f32 textureOffsetU = s_rcgpuState.cameraPos.x - sectorCached->floorOffset.x;
		f32 textureOffsetV = sectorCached->floorOffset.z - s_rcgpuState.cameraPos.z;

		f32 relFloor       = sectorCached->floorHeight - s_rcgpuState.eyeHeight;
		f32 scaledRelFloor = relFloor * s_rcgpuState.focalLenAspect;

		f32 cosScaledRelFloor = scaledRelFloor * s_rcgpuState.cosYaw;
		f32 negSinRelFloor    =-relFloor * s_rcgpuState.sinYaw;
		f32 sinScaledRelFloor = scaledRelFloor * s_rcgpuState.sinYaw;
		f32 negCosRelFloor    =-relFloor * s_rcgpuState.cosYaw;
	...
	{
		s_poly_offsetX = s_rcgpuState.cameraPos.x - offsetX;
		s_poly_offsetZ = offsetZ - s_rcgpuState.cameraPos.z;

		s_poly_scaledHOffset = heightOffset * s_rcgpuState.focalLenAspect;
		s_poly_sinYawHOffset = s_rcgpuState.sinYaw * heightOffset;
		s_poly_cosYawHOffset = s_rcgpuState.cosYaw * heightOffset;

		s_poly_cosYawScaledHOffset = s_rcgpuState.cosYaw * s_poly_scaledHOffset;
		s_poly_sinYawScaledHOffset = s_rcgpuState.sinYaw * s_poly_scaledHOffset;
	...
	void flat_drawPolygonScanline(s32 x0, s32 x1, s32 y, bool trans)
	{
		...
		const f32 right = f32(x1 - 1 - s_screenXMid) * s_rcgpuState.aspectScaleX;
		...
		const f32 worldTexelScaleAspect = yRcp * 8.0f * s_rcgpuState.aspectScaleY;
