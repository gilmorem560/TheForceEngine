...
namespace RClassic_Gpu
{
	...
	void robj3d_shadeVertices(s32 vertexCount, f32* outShading, const vec3_float* vertices, const vec3_float* normals)
	{
		...
		for (s32 i = 0; i < vertexCount; i++, normal++, vertex++, outShading++)
		{
			...
			else
			{
				...
				{
					const CameraLightGpu* light = &s_cameraLight[i];
	...
	{
		vec3_float offsetWS;
		offsetWS.x = fixed16ToFloat(obj->posWS.x) - s_rcgpuState.cameraPos.x;
		offsetWS.y = fixed16ToFloat(obj->posWS.y) - s_rcgpuState.eyeHeight;
		offsetWS.z = fixed16ToFloat(obj->posWS.z) - s_rcgpuState.cameraPos.z;

		// Calculate the view space object camera offset.
		vec3_float offsetVS;
		rotateVectorM3x3(&offsetWS, &offsetVS, s_rcgpuState.cameraMtx);

		// Concatenate the camera and object rotation matrices.
		f32 xform[9];
		robj3d_mulMatrix3x3(s_rcgpuState.cameraMtx, obj->transform, xform);
