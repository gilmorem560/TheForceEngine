...
#include "robj3dGpu.h"
#include "robj3dGpu_TransformAndLighting.h"
#include "robj3dGpu_PolygonSetup.h"
#include "robj3dGpu_Culling.h"
#include "robj3dGpu_Clipping.h"
#include "robj3dGpu_PolygonDraw.h"
...
namespace RClassic_Gpu
{
	...
	void robj3d_drawVertices(s32 vertexCount, const vec3_float* vertices, u8 color, s32 size)
	{
		...
		for (s32 v = 0; v < vertexCount; v++, vertex++)
		{
			...
			const s32 pixel_x = roundFloat((vertex->x*s_rcgpuState.focalLength)    / z + s_rcgpuState.projOffsetX);
			const s32 pixel_y = roundFloat((vertex->y*s_rcgpuState.focalLenAspect) / z + s_rcgpuState.projOffsetY);
			...
			if (z >= s_rcgpuState.depth1d[pixel_x] || pixel_y > s_windowMaxY_Pixels || pixel_y < s_windowMinY_Pixels || pixel_y < s_windowTop[pixel_x] || pixel_y > s_windowBot[pixel_x])
			{
				continue;
			}
	...
	void robj3d_projectVertices(vec3_float* pos, s32 count, vec3_float* out)
	{
		for (s32 i = 0; i < count; i++, pos++, out++)
		{
			...
			out->x = (f32)roundFloat((pos->x*s_rcgpuState.focalLength)   *rcpZ + s_rcgpuState.projOffsetX);
			out->y = (f32)roundFloat((pos->y*s_rcgpuState.focalLenAspect)*rcpZ + s_rcgpuState.projOffsetY);
			out->z = pos->z;
		}
	}
